...
async function getData() {
  try {
    const res = await fetch(
      "https://automatic-space-happiness-76w5wpxrw552xqpp-8000.app.github.dev/brief/Insight%20Agent%20Pro",
      { cache: "no-store" }
    );
    return res.json();
  } catch {
...
export default async function Home() {
  ...
  if (!data) {
    return (
      <main className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-cyan-950 text-white flex items-center justify-center">
        <p className="text-slate-400">Could not connect to backend.</p>
      </main>
    );
  }

  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-cyan-950 text-white p-8">
      <div className="max-w-5xl mx-auto">

        {/* Header */}
        <div className="mb-10">
          <p className="text-xs uppercase tracking-widest text-cyan-400 mb-2">Company Intelligence</p>
        ...
        <div className="bg-slate-800 rounded-2xl p-6 border border-slate-700 mb-6">
          ...
          <div className="space-y-3">
            {data.products?.map((item: string, i: number) => {
              const colors = ["bg-cyan-400","bg-violet-400","bg-emerald-400","bg-amber-400"];
              return (
                <div key={item}>
                  <span className="text-slate-200 text-sm">{item}</span>
                  <div className="w-full bg-slate-700 rounded-full h-2.5 mt-1">
                    <div className={"h-2.5 rounded-full " + colors[i % colors.length]} style={{ width: (85 - i * 10) + "%" }} />
                  </div>
                </div>
              );
            })}
